...
module.exports = function (logger, app, spinalAPIMiddleware) {
    app.post("/api/v1/endpoint/:id/timeSeries/insert", (req, res, next) => __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            var timeseries = yield spinalTimeSeries_1.default().getOrCreateTimeSeries(node.getId().get());
            const newValue = req.body.newValue;
            const date = moment(req.body.date, "DD MM YYYY HH:mm:ss", true);
            yield timeseries.insert(newValue, date.unix());
        }
        catch (error) {
            console.error(error);
            res.status(400).send();
